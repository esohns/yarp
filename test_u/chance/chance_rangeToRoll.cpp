...
print_usage (const std::string& programName_in)
{
  ...
  std::cout << ACE_TEXT_ALWAYS_CHAR ("currently available options:")
            << std::endl;
  std::cout << ACE_TEXT_ALWAYS_CHAR ("-p              : print all results")
            << std::endl;
  std::cout << ACE_TEXT_ALWAYS_CHAR ("-r [VALUE-VALUE]: range (e.g. 5-12)")
            << std::endl;
                   ...
                   bool& printVersionAndExit_out)
{
  ...
  while ((option = argumentParser()) != EOF)
  {
    switch (option)
    {
      ...
      case 'r':
      {
        std::string range = ACE_TEXT_ALWAYS_CHAR (argumentParser.opt_arg ());
...
void
do_work (const struct RPG_Dice_ValueRange& valueRange_in,
         RPG_Dice_Rolls_t& rolls_out)
{
  ...
  // basic algorithm
  enum RPG_Dice_DieType current_dieType = D_0;
  struct RPG_Dice_ValueRange current_range;
  do
  {
    current_dieType++;
    ...
    ACE_DEBUG ((LM_DEBUG,
               ...
               ACE_TEXT (RPG_Dice_DieTypeHelper::RPG_Dice_DieTypeToString (current_dieType).c_str ())));
  ...
  do
  {
    ...
    else
    {
      ...
      result.modifier = (std::max<int> (std::abs (current_range.begin),
                                        std::abs (current_range.end)) -
                         std::min<int> (std::abs (current_range.begin),
                                        std::abs (current_range.end))) / 2;
           ...
           ACE_TCHAR* argv_in[])
{
  ...
  // step2a: set defaults
  struct RPG_Dice_ValueRange value_range;
  value_range.begin = 0;
  value_range.end = 0;
...
//   } // end IF
  ...
  int index = 1;
  struct RPG_Dice_ValueRange range;
       ...
       iterator++, index++)
  {
    ...
    std::cout << ACE_TEXT_ALWAYS_CHAR ("[")
              << index
              << (perfect_match ? ACE_TEXT_ALWAYS_CHAR ("*]") : ACE_TEXT_ALWAYS_CHAR ("]"))
