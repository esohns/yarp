...
#PARAM_LIST="-a 0 -c -d 0 -m -r 10 -s -t -x 40 -y 20"
PARAM_LIST="-a 0 -c -d 0 -m -r 15 -s -x 80 -y 40"
...
[ ! -w ${DEF_LOG} ] && echo "ERROR: invalid logfile \"${DEF_LOG}\" (not writable), aborting" && exit 1

CMDLINE="${MAP_GENERATOR_EXEC} ${PARAM_LIST}"
for (( i=0; i<${NUM_RUNS}; i++ )) ; do
  echo "INFO: running iteration #${i}..."
  $(${CMDLINE} >>${DEF_OUT} 2>>${DEF_LOG})
