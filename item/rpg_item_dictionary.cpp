...
//   unsigned_byte_pimpl                       rangeIncrement_p;
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("::xml_schema::document::parse(): exception occurred: \"%s\", returning\n"),
                ACE_TEXT (text.c_str ())));
    return;
  } catch (...) {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("::xml_schema::document::parse(): exception occurred, returning\n")));
    return;
  }
                                    ...
                                    RPG_Item_PropertiesBase*& properties_out) const
{
  ...
  switch (item_in->type ())
  {
    case ITEM_ARMOR:
    {
      const RPG_Item_Armor* armor = static_cast<const RPG_Item_Armor*> (item_in);
      ACE_NEW_NORETURN (properties_out,
                        RPG_Item_ArmorProperties (getArmorProperties (armor->armorType_)));
      break;
    }
    case ITEM_COMMODITY:
    {
      const RPG_Item_Commodity* commodity = static_cast<const RPG_Item_Commodity*> (item_in);
      ACE_NEW_NORETURN (properties_out,
                        RPG_Item_CommodityProperties (getCommodityProperties (commodity->subtype_)));
      break;
    }
    case ITEM_OTHER:
    case ITEM_VALUABLE:
    ...
    case ITEM_WEAPON:
    {
      const RPG_Item_Weapon* weapon = static_cast<const RPG_Item_Weapon*> (item_in);
      ACE_NEW_NORETURN (properties_out,
                        RPG_Item_WeaponProperties (getWeaponProperties (weapon->weaponType_)));
      break;
    }
    default:
    {
      ACE_DEBUG ((LM_DEBUG,
                  ...
                  ACE_TEXT (RPG_Item_TypeHelper::RPG_Item_TypeToString (item_in->type ()).c_str ())));
      break;
    }
  } // end SWITCH
...
RPG_Item_Dictionary::getCommodityProperties (const struct RPG_Item_CommodityUnion& commoditySubType_in) const
{
  ...
  switch (commoditySubType_in.discriminator)
  {
    ...
    {
      RPG_Item_CommodityBeverageDictionaryIterator_t iterator =
        myCommodityBeverageDictionary.find (commoditySubType_in.commoditybeverage);
      if (iterator == myCommodityBeverageDictionary.end())
      {
        ACE_DEBUG ((LM_DEBUG,
                    ACE_TEXT ("invalid commodity beverage type \"%s\", aborting\n"),
                    ACE_TEXT (RPG_Item_CommodityBeverageHelper::RPG_Item_CommodityBeverageToString (commoditySubType_in.commoditybeverage).c_str ())));
        break;
      } // end IF
    ...
    {
      RPG_Item_CommodityLightDictionaryIterator_t iterator =
        myCommodityLightDictionary.find (commoditySubType_in.commoditylight);
      if (iterator == myCommodityLightDictionary.end ())
      {
        ACE_DEBUG ((LM_DEBUG,
                    ACE_TEXT ("invalid commodity light type \"%s\", aborting\n"),
                    ACE_TEXT (RPG_Item_CommodityLightHelper::RPG_Item_CommodityLightToString (commoditySubType_in.commoditylight).c_str ())));
        break;
      } // end IF
  ...
  ACE_ASSERT (false);
  struct RPG_Item_CommodityProperties dummy;
...
RPG_Item_Dictionary::getArmorProperties (enum RPG_Item_ArmorType armorType_in) const
{
  ...
  if (iterator == myArmorDictionary.end ())
  {
    ACE_DEBUG ((LM_DEBUG,
                ...
                ACE_TEXT (RPG_Item_ArmorTypeHelper::RPG_Item_ArmorTypeToString (armorType_in).c_str ())));

//    ACE_ASSERT(false);
    struct RPG_Item_ArmorProperties dummy;
    dummy.aura = RPG_MAGIC_SCHOOL_INVALID;
    dummy.baseStorePrice.numGoldPieces = 0;
    dummy.baseStorePrice.numSilverPieces = 0;
    dummy.baseWeight = 0;
...
RPG_Item_Dictionary::getWeaponProperties (enum RPG_Item_WeaponType weaponType_in) const
{
  ...
  if (iterator == myWeaponDictionary.end ())
  {
    ACE_DEBUG ((LM_DEBUG,
                ...
                ACE_TEXT (RPG_Item_WeaponTypeHelper::RPG_Item_WeaponTypeToString (weaponType_in).c_str ())));

    ACE_ASSERT (false);
    struct RPG_Item_WeaponProperties dummy;
    dummy.aura = RPG_MAGIC_SCHOOL_INVALID;
    dummy.baseStorePrice.numGoldPieces = 0;
    dummy.baseStorePrice.numSilverPieces = 0;
    dummy.baseWeight = 0;
...
RPG_Item_Dictionary::dump () const
{
       ...
       iterator++)
  {
    ACE_DEBUG ((LM_DEBUG,
                ...
                ACE_TEXT (RPG_Item_WeaponTypeHelper::RPG_Item_WeaponTypeToString ((*iterator).first).c_str ()),
                ACE_TEXT (RPG_Item_WeaponCategoryHelper::RPG_Item_WeaponCategoryToString ((*iterator).second.category).c_str ()),
                ACE_TEXT (RPG_Item_WeaponClassHelper::RPG_Item_WeaponClassToString ((*iterator).second.weaponClass).c_str ()),
                (*iterator).second.baseStorePrice.numGoldPieces,
                (*iterator).second.baseStorePrice.numSilverPieces,
                ACE_TEXT (RPG_Dice_Common_Tools::toString ((*iterator).second.baseDamage).c_str ()),
                ...
                ACE_TEXT (RPG_Item_Common_Tools::toString ((*iterator).second.typeOfDamage).c_str ())));
       ...
       iterator++)
  {
    ACE_DEBUG ((LM_DEBUG,
                ACE_TEXT ("Item: Armor\nType: %s\nCategory: %s\nPrice: %d GP, %d SP\nAC Bonus: %d\nmax Dex Bonus: %d\nPenalty: %d\nSpell Failure: %d%%\nSpeed: %d\nWeight: %d\n"),
                ACE_TEXT (RPG_Item_ArmorTypeHelper::RPG_Item_ArmorTypeToString ((*iterator).first).c_str ()),
                ACE_TEXT (RPG_Item_ArmorCategoryHelper::RPG_Item_ArmorCategoryToString ((*iterator).second.category).c_str ()),
                (*iterator).second.baseStorePrice.numGoldPieces,
                (*iterator).second.baseStorePrice.numSilverPieces,
