...
XML_Handler::startDocument ()
{
  ...
  if (!myFilePerDefinition)
  {
    ...
    if (myCurrentOutputFile.fail () || !myCurrentOutputFile.is_open ())
    {
      ...
      XMLCh* message = XMLString::transcode (ACE_TEXT_ALWAYS_CHAR ("failed to create/open file, aborting\n"));
      ACE_ASSERT (message);
    ...
    insertMultipleIncludeProtection (false,
                                     targetFilename,
                                     myCurrentOutputFile);
  } // end IF
...
//                  ACE_TEXT(myFilename.c_str())));
          ...
          insertPreamble(myCurrentOutputFile);
          const ACE_TCHAR* basename = ACE::basename (FQfilename.c_str (), ACE_DIRECTORY_SEPARATOR_CHAR);
          if (!basename)
          {
            ACE_DEBUG ((LM_ERROR,
        ...
        {
          std::string filename =
            ACE_TEXT_ALWAYS_CHAR (ACE::basename (FQfilename.c_str (), ACE_DIRECTORY_SEPARATOR_CHAR));
...
//      } // end IF
      ...
      if (typeOfElement == XML_ELEMENT)
      {
        ...
        definition += ACE_TEXT_ALWAYS_CHAR (" ");
        definition += (minOccurs ? minOccurs
                                 : ACE_TEXT_ALWAYS_CHAR (XML2CPPCODE_DEFAULTMINOCCURS));
        XMLString::release (&minOccurs);
        definition += ACE_TEXT_ALWAYS_CHAR (" ");
        std::string string_maxoccurs = (maxOccurs ? maxOccurs
...
void
XML_Handler::insertMultipleIncludeProtection (bool usePragmaOnce_in,
