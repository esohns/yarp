...
{
  RPG_TRACE (ACE_TEXT ("RPG_Engine_Common_Tools::initialize"));
...
{
  RPG_TRACE (ACE_TEXT ("RPG_Engine_Common_Tools::finalize"));
...
RPG_Engine_Common_Tools::createEntity ()
{
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to RPG_Player::random(), aborting\n")));
    return result;
  } // end IF
...
RPG_Engine_Common_Tools::info (const struct RPG_Engine_Entity& entity_in)
{
  ...
  result += ACE_TEXT_ALWAYS_CHAR ("entity: ");
  result += entity_in.character->getName();
  result += ACE_TEXT_ALWAYS_CHAR ("\n===============\n");

  std::ostringstream converter;
  result += ACE_TEXT_ALWAYS_CHAR ("position: [");
  ...
  result += ACE_TEXT_ALWAYS_CHAR ("mode(s) [");
  converter.str (ACE_TEXT (""));
  ...
  result += ACE_TEXT_ALWAYS_CHAR ("\\end mode(s)\n");
...
RPG_Engine_Common_Tools::generateStandardItems (enum RPG_Common_SubClass subClass_in)
{
  ...
  RPG_Item_Instance_Manager* manager_p =
    RPG_ITEM_INSTANCE_MANAGER_SINGLETON::instance ();
  ...
  try {
    switch (subClass_in)
    {
      ...
      {
        current = manager_p->create (ITEM_WEAPON,
                                     ONE_HANDED_MELEE_WEAPON_SWORD_LONG);
        ACE_ASSERT (current);
        result.insert (current->id ());
        current = manager_p->create (ITEM_ARMOR,
                                     ARMOR_MAIL_SPLINT);
        ACE_ASSERT (current);
        result.insert (current->id ());
        current = manager_p->create (ITEM_ARMOR,
