...
//   props.schema_location("http://www.w3.org/XML/1998/namespace", "xml.xsd");
  ...
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("engine_level_t(\"%s\"): exception occurred: \"%s\", aborting\n"),
                ACE_TEXT (filename_in.c_str ()),
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("engine_level_t(\"%s\"): exception occurred, aborting\n"),
                ACE_TEXT (filename_in.c_str ())));
    return false;
  }
  if (is_level)
                        ...
                        const struct RPG_Engine_LevelData& level_in)
{
  ...
  if (Common_File_Tools::isReadable (filename_in))
    ACE_DEBUG ((LM_INFO,
                ACE_TEXT ("overwriting existing file (\"%s\"), continuing\n"),
                ACE_TEXT (filename_in.c_str ())));
  ...
  map[ACE_TEXT_ALWAYS_CHAR ("")].name = ACE_TEXT_ALWAYS_CHAR (RPG_COMMON_XML_TARGET_NAMESPACE);
  map[ACE_TEXT_ALWAYS_CHAR ("")].schema = ACE_TEXT_ALWAYS_CHAR (RPG_ENGINE_SCHEMA_FILE);
  try {
    ::engine_level_t (ofs,
                      *level_xml_p,
                      map,
  ...
  delete level_xml_p; level_xml_p = NULL;
...
error:
  try {
    ...
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to std::ofstream::close(\"%s\"): \"%s\", aborting\n"),
                ACE_TEXT (filename_in.c_str ()), ACE_TEXT (exception.what ())));
  } catch (...) {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to std::ofstream::close(\"%s\"), aborting\n"),
                ACE_TEXT (filename_in.c_str ())));
...
RPG_Engine_Level::print (const struct RPG_Engine_LevelData& level_in)
{
  ...
  ACE_DEBUG ((LM_INFO,
              ...
              (level_in.metadata.environment.outdoors ? ACE_TEXT ("true") : ACE_TEXT ("false"))));
  ACE_DEBUG ((LM_INFO,
...
void
RPG_Engine_Level::initialize (const struct RPG_Engine_LevelData& level_in)
{
  RPG_TRACE (ACE_TEXT ("RPG_Engine_Level::initialize"));
...
RPG_Engine_Level::levelXMLToLevel (const RPG_Engine_Level_XMLTree_Type& level_in)
{
  ...
  const RPG_Engine_Level_XMLTree_Type::spawn_sequence& spawns =
      level_in.spawn ();
  for (RPG_Engine_Level_XMLTree_Type::spawn_const_iterator iterator = spawns.begin ();
       iterator != spawns.end ();
       iterator++)
  {
