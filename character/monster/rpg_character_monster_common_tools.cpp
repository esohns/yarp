...
// init statics
RPG_Character_Monster_NaturalWeaponToStringTable_t RPG_Character_Monster_NaturalWeaponHelper::myRPG_Character_Monster_NaturalWeaponToStringTable;
RPG_Character_Monster_OrganizationToStringTable_t RPG_Character_Monster_OrganizationHelper::myRPG_Character_Monster_OrganizationToStringTable;

void
RPG_Character_Monster_Common_Tools::initStringConversionTables()
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::initStringConversionTables"));

  RPG_Character_Monster_NaturalWeaponHelper::init();
  RPG_Character_Monster_OrganizationHelper::init();
...
const std::string
RPG_Character_Monster_Common_Tools::weaponTypeToString(const RPG_Character_Monster_WeaponTypeUnion& weaponType_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::weaponTypeToString"));
  ...
  switch (weaponType_in.discriminator)
  {
    ...
    {
      return RPG_Character_Monster_NaturalWeaponHelper::RPG_Character_Monster_NaturalWeaponToString(weaponType_in.naturalweapon);
    }
    case RPG_Character_Monster_WeaponTypeUnion::ABILITY:
    {
    ...
    case RPG_Character_Monster_WeaponTypeUnion::SPECIALATTACK:
    {
...
const std::string
RPG_Character_Monster_Common_Tools::attackActionToString(const RPG_Character_Monster_AttackAction& attackAction_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::attackActionToString"));

  std::string result;

  result += ACE_TEXT_ALWAYS_CHAR("weapon: ");
  result += RPG_Character_Monster_Common_Tools::weaponTypeToString(attackAction_in.weapon);
...
const std::string
RPG_Character_Monster_Common_Tools::attackToString(const RPG_Character_Monster_Attack& attack_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::attackToString"));
  ...
  int i = 1;
  for (std::vector<RPG_Character_Monster_AttackAction>::const_iterator iterator = attack_in.standardAttackActions.begin();
  ...
  i = 1;
  for (std::vector<RPG_Character_Monster_AttackAction>::const_iterator iterator = attack_in.fullAttackActions.begin();
...
const std::string
RPG_Character_Monster_Common_Tools::organizationsToString(const RPG_Character_Monster_OrganizationSet_t& organizations_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::organizationsToString"));

  std::string result;
  for (RPG_Character_Monster_OrganizationSetIterator_t iterator = organizations_in.begin();
       iterator != organizations_in.end();
       iterator++)
  {
    result += RPG_Character_Monster_OrganizationHelper::RPG_Character_Monster_OrganizationToString(*iterator);
    result += ACE_TEXT_ALWAYS_CHAR("|");
  } // end FOR
...
const std::string
RPG_Character_Monster_Common_Tools::organizationsToString(const RPG_Character_Monster_Organizations_t& organizations_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::organizationsToString"));

  std::string result;
  std::string range_string;
  for (RPG_Character_Monster_OrganizationsIterator_t iterator = organizations_in.begin();
       iterator != organizations_in.end();
       iterator++)
  {
    result += RPG_Character_Monster_OrganizationHelper::RPG_Character_Monster_OrganizationToString((*iterator).type);
...
const std::string
RPG_Character_Monster_Common_Tools::advancementToString(const RPG_Character_Monster_Advancement_t& advancement_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::advancementToString"));

  std::string result;
  for (RPG_Character_Monster_AdvancementIterator_t iterator = advancement_in.begin();
...
RPG_Character_Monster_Common_Tools::naturalWeaponToPhysicalDamageType(const RPG_Character_Monster_NaturalWeapon& naturalWeapon_in)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::naturalWeaponToPhysicalDamageType"));
...
//       result.push_back(PHYSICALDAMAGE_PIERCING);
    ...
    default:
    {
      ACE_DEBUG((LM_ERROR,
                 ...
                 RPG_Character_Monster_NaturalWeaponHelper::RPG_Character_Monster_NaturalWeaponToString(naturalWeapon_in).c_str()));
...
void
RPG_Character_Monster_Common_Tools::generateRandomEncounter(const unsigned int& numDifferentMonsterTypes_in,
                                                  const unsigned int& numMonsters_in,
                                                  const RPG_Character_Alignment& alignment_in,
                                                  const RPG_Common_Environment& environment_in,
                                                  const RPG_Character_Monster_OrganizationSet_t& organizations_in,
                                                  RPG_Character_Monster_Encounter_t& encounter_out)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::generateRandomEncounter"));

  // init result
  encounter_out.clear();

  // sanity check(s)
  ACE_ASSERT(numDifferentMonsterTypes_in <= RPG_CHARACTER_MONSTER_DICTIONARY_SINGLETON::instance()->numEntries());
  if (numMonsters_in)
  {
    ACE_ASSERT(numDifferentMonsterTypes_in <= numMonsters_in);
  } // end IF

  RPG_Character_Monster_List_t list;
  RPG_CHARACTER_MONSTER_DICTIONARY_SINGLETON::instance()->find(alignment_in,
                                                     environment_in,
                                                     organizations_in,
                                                     list);
  if (list.empty())
  {
    ...
    ACE_DEBUG((LM_DEBUG,
               ...
               RPG_CHARACTER_MONSTER_DICTIONARY_SINGLETON::instance()->numEntries()));
  ...
  RPG_Character_Monster_OrganizationSet_t possible_organizations;
  for (unsigned int i = 0;
       i < numDifferentMonsterTypes_in;
       i++)
  {
    // step2a: choose new random foe (from the set of possibilities)
    RPG_Character_Monster_EncounterConstIterator_t iterator;
    int choiceType = 0;
    do
    {
      result.clear();
    ...
    properties = RPG_CHARACTER_MONSTER_DICTIONARY_SINGLETON::instance()->getProperties(list[choiceType]);
    for (RPG_Character_Monster_OrganizationsIterator_t iterator2 = properties.organizations.begin();
    ...
    if (organizations_in.find(ORGANIZATION_ANY) == organizations_in.end())
    {
      ...
      std::vector<RPG_Character_Monster_Organization> intersection;
      std::set_intersection(organizations_in.begin(),
                            ...
                            intersection.begin());
      possible_organizations.clear();
      for (std::vector<RPG_Character_Monster_Organization>::const_iterator iterator3 = intersection.begin();
    ...
    RPG_Character_Monster_OrganizationSetIterator_t iterator2 = possible_organizations.begin();
    std::advance(iterator2, choiceOrganization);
    RPG_Character_Monster_OrganizationsIterator_t iterator3 = properties.organizations.begin();
    while ((*iterator3).type != *iterator2)
      iterator3++;
    ACE_ASSERT(iterator3 != properties.organizations.end());
    RPG_Dice_Roll roll;
    RPG_Character_Monster_Common_Tools::organizationStepToRoll(*iterator3,
                                                     roll);
    result.clear();
  ...
  if (numMonsters_in)
  {
    for (RPG_Character_Monster_EncounterConstIterator_t iterator = encounter_out.begin();
...
void
RPG_Character_Monster_Common_Tools::organizationStepToRoll(const RPG_Character_Monster_OrganizationStep& organizationStep_in,
                                                 RPG_Dice_Roll& roll_out)
{
  RPG_TRACE(ACE_TEXT("RPG_Character_Monster_Common_Tools::organizationStepToRoll"));
  ...
  switch (organizationStep_in.type)
  {
    ...
    default:
    {
      ACE_DEBUG((LM_ERROR,
                 ...
                 RPG_Character_Monster_OrganizationHelper::RPG_Character_Monster_OrganizationToString(organizationStep_in.type).c_str()));
