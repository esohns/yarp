...
std::string
RPG_Monster_Common_Tools::toString (const struct RPG_Monster_WeaponTypeUnion& weaponType_in)
{
  RPG_TRACE (ACE_TEXT ("RPG_Monster_Common_Tools::toString"));
...
std::string
RPG_Monster_Common_Tools::toString (const struct RPG_Monster_AttackAction& attackAction_in)
{
  RPG_TRACE (ACE_TEXT ("RPG_Monster_Common_Tools::toString"));
...
std::string
RPG_Monster_Common_Tools::toString (const struct RPG_Monster_Attack& attack_in)
{
  RPG_TRACE (ACE_TEXT ("RPG_Monster_Common_Tools::toString"));
  ...
  int i = 1;
  for (std::vector<struct RPG_Monster_AttackAction>::const_iterator iterator = attack_in.standardAttackActions.begin ();
  ...
  i = 1;
  for (std::vector<struct RPG_Monster_AttackAction>::const_iterator iterator = attack_in.fullAttackActions.begin ();
...
RPG_Monster_Common_Tools::naturalWeaponToPhysicalDamageType (enum RPG_Monster_NaturalWeapon naturalWeapon_in)
{
  ...
  switch (naturalWeapon_in)
  {
    ...
    case NATURALWEAPON_STING:
      result.insert (PHYSICALDAMAGE_PIERCING); break;
...
//       result.push_back(PHYSICALDAMAGE_PIERCING);
    ...
    case NATURALWEAPON_TENTACLE:
      result.insert (PHYSICALDAMAGE_BLUDGEONING); break;
    case NATURALWEAPON_SPIT:
    case NATURALWEAPON_WEB:
      result.insert (PHYSICALDAMAGE_NONE); break;
    default:
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("invalid natural weapon: \"%s\", aborting\n"),
                  ACE_TEXT (RPG_Monster_NaturalWeaponHelper::RPG_Monster_NaturalWeaponToString (naturalWeapon_in).c_str ())));
      break;
    }
  } // end SWITCH
...
void
RPG_Monster_Common_Tools::generateRandomEncounter (unsigned int numDifferentMonsterTypes_in,
                                                   unsigned int numMonsters_in,
                                                   const struct RPG_Character_Alignment& alignment_in,
                                                   const struct RPG_Common_Environment& environment_in,
                                                   ...
                                                   RPG_Monster_Encounter_t& encounter_out)
{
  ...
  if (list.empty ())
  {
    ...
    ACE_DEBUG ((LM_ERROR,
                ...
                ACE_TEXT (RPG_Monster_Common_Tools::toString (organizations_in).c_str ()),
                ACE_TEXT (RPG_Dice_Common_Tools::toString (hitDice_in).c_str ()),
       ...
       i++)
  {
    ...
    properties =
      RPG_MONSTER_DICTIONARY_SINGLETON::instance ()->getProperties (list[choiceType]);
    ...
    if (organizations_in.find (ORGANIZATION_ANY) == organizations_in.end ())
    {
      ...
      std::vector<enum RPG_Monster_Organization> intersection;
      std::set_intersection (organizations_in.begin (),
                             ...
                             intersection.begin ());
      possible_organizations.clear ();
      for (std::vector<enum RPG_Monster_Organization>::const_iterator iterator3 = intersection.begin ();
  ...
  if (numMonsters_in)
  {
    ...
    int diff = std::abs (static_cast<int> (numCurrentFoes) - static_cast<int> (numMonsters_in));
    while (diff)
    {
  ...
  ACE_DEBUG ((LM_DEBUG,
              ACE_TEXT ("generated %u foes...\n"),
              numCurrentFoes));
}

void
RPG_Monster_Common_Tools::organizationStepToRoll (const struct RPG_Monster_OrganizationStep& organizationStep_in,
                                                  struct RPG_Dice_Roll& roll_out)
{
  ...
  switch (organizationStep_in.type)
  {
    case ORGANIZATION_SOLITARY:
      roll_out.modifier = 1; break;
    case ORGANIZATION_PAIR:
      roll_out.modifier = 2; break;
    ...
    default:
    {
      ACE_DEBUG ((LM_ERROR,
                  ...
                  ACE_TEXT (RPG_Monster_OrganizationHelper::RPG_Monster_OrganizationToString (organizationStep_in.type).c_str ())));
      break;
    }
  } // end SWITCH
}
