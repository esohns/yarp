...
%XML2CppCodeEXE% -d RPG_Client_Export -e -f .\..\rpg_client.xsd -i -o .\.. -s -u -x RPG_Client
if %ERRORLEVEL% NEQ 0 (
...
"%XsdEXE%" cxx-tree --generate-serialization --generate-ostream --generate-comparison --type-regex "/(.+) RPG_(.+)_Type/RPG_\u$2_XMLTree_Type/" --char-type char --output-dir .\.. --namespace-map urn:rpg= --root-element-all --extern-xml-schema rpg_XMLSchema.h --hxx-suffix _XML_tree.h --cxx-suffix _XML_tree.cpp --show-anonymous --show-sloc --export-symbol "RPG_Client_Export" --hxx-prologue "#include \"rpg_client_exports.h\"" --cxx-prologue-file .\..\stdafx.cpp ..\rpg_client.xsd
if %ERRORLEVEL% NEQ 0 (
