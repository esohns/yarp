...
RPG_Client_Engine::handleActions()
{
  ...
  RPG_Client_Action current_action = myActions.front();
  switch (current_action.command)
  {
    case COMMAND_CURSOR_DRAW:
    {
      SDL_Rect dirtyRegion;
      RPG_GRAPHICS_CURSOR_MANAGER_SINGLETON::instance()->put(current_action.position.first,
                                                             current_action.position.second,
                                                             current_action.window->getScreen(),
                                                             dirtyRegion);
    ...
    {
      SDL_Rect dirtyRegion;
      RPG_GRAPHICS_CURSOR_MANAGER_SINGLETON::instance()->restoreBG(current_action.window->getScreen(),
                                                                   dirtyRegion);
      RPG_Graphics_Surface::update(dirtyRegion,
                                   current_action.window->getScreen());
...
/*    case COMMAND_REFRESH:
    ...
    {
      RPG_Client_WindowLevel* window = dynamic_cast<RPG_Client_WindowLevel*>(current_action.window);
      ACE_ASSERT(window);
      try
      {
        window->toggleDoor(current_action.position);
      }
      catch (...)
      {
        ACE_DEBUG((LM_CRITICAL,
